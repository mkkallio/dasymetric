...
compute_area_weights <- function(source, 
    ...
    if(test) {
        ...
        if(!test) stop("No column ", pycno," in source")
    ...
    target <- suppressWarnings(source %>% 
        dplyr::select(sourceID, pycno = !!pycno) %>%
   ...
   if(pycnophylactic) {
        ...
        if(test) {
            sourceareas <- source$area
        } else {
            sourceareas <- units::set_units(st_area(source), "km2")
        }
        
        if(dasymetric) {
            dasymetric_var <- dplyr::pull(target, dasy)
            pycno_res <- dasymetric:::iterate_pycno(target, 
                                       dasy = dasymetric_var,
                                       touching, 
                                       boundary,
                                       pycno_var, 
